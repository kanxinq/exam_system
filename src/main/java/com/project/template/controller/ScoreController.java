...
//方法返回一个封装了学生成绩表列表的统一响应结果对象，通过调用服务层的list方法获取学生成绩表列表并封装到响应结果中
    /**
     * 分页查询
     */
...
//方法接收一个学生成绩表实体对象作为请求体参数，通过调用服务层的save方法保存学生成绩表数据，并返回一个封装了成功信息的统一响应结果对象
    /**
     * 数据更新
     */
...
//方法接收一个学生成绩表实体对象作为请求体参数，通过调用服务层的updateById方法更新学生成绩表数据，并返回一个封装了成功信息的统一响应结果对象
    /**
     * 根据id删除
     */
...
//方法接收一个包含学生成绩表 ID 的列表作为请求体参数，通过调用服务层的removeByIds方法批量删除学生成绩表数据，并返回一个封装了成功信息的统一响应结果对象
    /**
     * 根据id查询
     */
...
//    方法接收学生成绩表 ID 作为请求参数，通过调用服务层的getById方法根据 ID 查询学生成绩表数据，并返回一个封装了查询结果的统一响应结果对象
}

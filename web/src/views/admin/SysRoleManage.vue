...
export default {
    ...
    methods: {
            ...
            submitDrawer() {
                //新增
                if (!this.formData.id) {
                    this.$axios.post("/sysRole/add",this.formData).then(res => {
                        console.log(res)
                        if (res.code === 200) {
                            this.isDrawerDialog = false
                            this.$message({
                    ...
                    //更新
                    this.$axios.put("/sysRole/update", this.formData).then(res => {
                        console.log(res)
                        if (res.code === 200) {
                            this.isDrawerDialog = false
                            this.$message({
            ...
            batchDelete() {
                this.$confirm(`此操作将永久删除ID为[${this.ids}]的数据, 是否继续?`, '提示', {
                    ...
                    //批量删除数据
                    this.$axios.delete("/sysRole/delBatch" ,{data:this.ids}).then(res => {
                        if (res.code === 200) {
                            this.$message({
                                message: res.msg,
                                type: 'success'
